...
(function() {
    ...
    var mkdir = function(dir) {
        return function(callback) {
            //process.stdout.write('Go Make ' + dir + '/ ... \n');
    ...
    var writefile = function(filename, retmpl, projectInfo) {
        return function(callback) {
            //process.stdout.write('Go Write ' + filename + ' ... \n');
    ...
    var scaffolding = function(projectInfo, globalCallback) {
        ...
        async.each(files, function(repo, callback) {
            ...
            if (!projectInfo.exCoffee && repo.indexOf('coffee') !== -1) {
                return;
            }
            if (!projectInfo.exES6 && repo.indexOf('es6') !== -1) {
                return;
            }
            if (!projectInfo.exLess && repo.indexOf('less') !== -1) {
                return;
            }
            if (!projectInfo.exJade && repo.indexOf('jade') !== -1) {
                return;
            }
        ...
        prompt.get(schema1, function(error, res) {
            ...
            prompt.get(schema2, function(error, result) {
                ...
                inquirer.prompt(schema3, function(answers) {
                    ...
                    result.scaffold = answers.check === "NO" ? "none" : "default";
